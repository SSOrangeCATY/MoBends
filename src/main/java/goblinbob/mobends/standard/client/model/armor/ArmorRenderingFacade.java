...
public class ArmorRenderingFacade
{
    ...
    public <T extends LivingEntity> boolean renderModelToConsumer(
            ArmorRenderContext<T> context,
            Model<?> armorModel,
            RenderType renderType)
    {
        if (context == null || armorModel == null || renderType == null || context.getSubmitNodeCollector() == null)
        {
            return false;
        }

        context.getSubmitNodeCollector().submitCustomGeometry(context.getPoseStack(), renderType, (pose, vertexConsumer) ->
        {
            PoseStack submittedPoseStack = new PoseStack();
            submittedPoseStack.last().set(pose);
            renderModelToConsumer(context, armorModel, vertexConsumer, submittedPoseStack);
        });
        return true;
    }
    ...
    public <T extends LivingEntity> boolean renderModelToConsumer(
            ...
            PoseStack poseStack)
    {
        if (context == null || armorModel == null || vertexConsumer == null || poseStack == null)
        {
            return false;
        }
        ...
        {
            success = tier1Renderer.renderToConsumer(context, humanoidModel, vertexConsumer, poseStack, true);
            if (!success)
            {
                success = tier2Renderer.renderToConsumer(context, armorModel, vertexConsumer, poseStack, true);
            }
        }
        else
        {
